...
dataInput.forEach((polycyPassword, index) => {
  let splitPolicyFromPassword = polycyPassword.split(':');
  let splitRuleFromChar = splitPolicyFromPassword[0].split(' ');
  let regexStringLimits = splitRuleFromChar[0].split('-');
  let regexString = new RegExp(/${splitRuleFromChar[1]}/, 'gi');

  // Part 1
  // let occurs = splitPolicyFromPassword[1].trim().match(regexString)
  //   ? splitPolicyFromPassword[1].trim().match(regexString).length
  //   : 0;

  // if (regexStringLimits[0] <= occurs && occurs <= regexStringLimits[1]) {
  //   validPasswordsCounter++;
  // }

  // Part 2
  let isAtPosition1 = splitPolicyFromPassword[1].trim().charAt(regexStringLimits[0] - 1) == splitRuleFromChar[1];
  let isAtPosition2 = splitPolicyFromPassword[1].trim().charAt(regexStringLimits[1] - 1) == splitRuleFromChar[1];

  if ((isAtPosition1 && !isAtPosition2) || (!isAtPosition1 && isAtPosition2)) {
    validPasswordsCounter++;
  }
});

console.log(validPasswordsCounter); // 393 is not the answers
